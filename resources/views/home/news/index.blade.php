        ...
        <div>
            @foreach($news as $new)
                <div class="blog-post">
                    ...
                    <p class="blog-post-meta">时间:{{$new->created_at->toFormattedDateString()}} 作者: <a href="/user/{{$new->user->id}}">{{$new->user->name}}</a>
                    </p>
                    {!! str_limit($new->description, 100, '...') !!}
                    <p class="blog-post-meta">赞 {{$new->zans_count}} | 评论 {{$new->comments_count}}</p>
                </div>
            @endforeach
            {{$news->links()}}
        </div><!-- /.blog-main -->
